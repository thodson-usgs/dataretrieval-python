...
- **Breaking:** a Water Use fan-out interrupted by a 5xx or 429 now raises
  ...
  unaffected, but a narrow ``except ServiceUnavailable`` around a Water Use call
  must widen. This is convergence, not novelty -- it is what the OGC getters
  have always done -- and it is what makes the failure resumable.

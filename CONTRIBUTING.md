...
complexipy --max-complexity-allowed 25 --failed dataretrieval
lint-imports
```
...
can forbid an edge, never require one), and package-wide cycle detection.
Import Linter's `acyclic_siblings` contract does not see cycles between a
package facade and one of its descendants, so acyclicity remains an AST fitness
function. If you are adding a rule and it is purely "module A must not import
